...
**MVVM pattern** with six projects, layered for platform portability:
...
**Data flow**: Models (`TD`) + Localization (`TD.i18n`) → Repository Contracts (`TD.RepositoryContracts`) → SQLite repositories (`TD.SQLite`) → Services (`TD.WPF`) → ViewModels (inherit from `TD.AppInfra`) → XAML Views
...
**Platform strategy**: Core models (`TD`), shared MVVM infrastructure (`TD.AppInfra`), repository contracts (`TD.RepositoryContracts`), localization (`TD.i18n`), and data access (`TD.SQLite`) target `net10.0` (cross-platform). Only the WPF project targets `net10.0-windows10.0.19041.0`. Keep business logic, contracts, data access, and MVVM base classes out of platform-specific projects so future frontends (MAUI, web, etc.) can reuse them.
...
- Shared base classes live in `TD.AppInfra` (`AbstractViewModel`, `AbstractDataErrorInfoVM`, `AbstractCreateEditVM`, `AbstractCollectionVM`, `AbstractDiaryVM`, etc.) with `public` visibility for cross-platform reuse
- Shared command bases live in `TD.AppInfra.Commands` (`AbstractCommand`, `AbstractAsyncCommand`) and should be reused by platform-specific UI projects.
- WPF-specific ViewModels live in `TD.WPF` and inherit from `TD.AppInfra` base classes
...
- **Custom validation attributes** live in `TD.Validation` and are applied via OzzCodeGen on generated model properties:
  ...
  - `NotGreaterThanPropertyAttribute(string comparisonPropertyName)` — validates that the decorated property's value does not exceed another property's value on the same object. Null on either side is treated as valid. Use on `FilledQuantity` to cap it against `OrderQuantity` on order entities. OzzCodeGen places this attribute directly on the generated property so no buddy class is needed.
