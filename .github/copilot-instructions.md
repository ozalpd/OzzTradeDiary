...
**MVVM pattern** with five projects, layered for platform portability:
...
**Data flow**: Models (`TD`) + Localization (`TD.i18n`) → SQLite repositories (`TD.SQLite`) → Services (`TD.WPF`) → ViewModels (inherit from `TD.AppInfra`) → XAML Views
...
**Platform strategy**: Core models (`TD`), shared MVVM infrastructure (`TD.AppInfra`), localization (`TD.i18n`), and data access (`TD.SQLite`) target `net10.0` (cross-platform). Only the WPF project targets `net10.0-windows10.0.19041.0`. Keep business logic, data access, and MVVM base classes out of platform-specific projects so future frontends (MAUI, web, etc.) can reuse them.
